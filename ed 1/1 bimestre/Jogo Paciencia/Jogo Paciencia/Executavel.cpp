...
//Função que vai pegar os valores que está na struct e colocar tudo na pilha
...
TpPilhaM1 comprarCartas(TpPilhaM1 &PM1){// Lembrar de mudar essa funçao , fazer de modo que voce apenas chame ela e faça a repetiçao em outro luga
    TpBaralho carta,aux;
	int numCompras = 0;
    char opc = ' ';
    ExibirPM1(PM1,1);
...
void validaPilhas(TpPilhaM &pm2, int npO, int npD) // O=ORIGEM | D=DESTINO
{
    ...
    // Exibir mensagem que a opção que ele digitou não é válida
}
...
int main(void)
{
    ...
    */
    ...
    int npBaralho = 11, npc=0, npO, npD; // controle da pilha que estamos mechendo - np coemça de 11 pela conta na inicializaçao da pilha
    int game = 1;
